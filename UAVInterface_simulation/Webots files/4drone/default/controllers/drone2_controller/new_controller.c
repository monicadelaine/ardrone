...
static int fd;
static fd_set rfds;
double coords[3] = {-19.9352,4.21,-10.9403};
...
void leftTurn(double altitude, float x)
{
  if (dofState != 0){
...
void rightTurn(double altitude, float x)
{
...
void goToPos()
{
	...
	float a, turn, dist;
	float m = waypoints[head+2] - coords[2];
	float n = waypoints[head+1] - coords[0];
	...
	//angle to turn
	turn = a - getAngle();
	dofSelection(RIGHT, turn); //add a check whether to turn right or left

	//distance to travel to waypoint
	dist = sqrt(pow(waypoints[head+1] - coords[0], 2) + pow(waypoints[head+2] - coords[2], 2));
...
void randomFly() 
{
	...
	dofSelection(RIGHT, (rand() % turnAngle)); 
	dofSelection(FORWARD, (rand() % 5));
}
