...
#define REMOTE_SERVER_PORT 1511 //video
#define REMOTE_SERVER_PORT_WP 1512 //rec wp
#define REMOTE_SERVER_PORT2 1501
#define REMOTE_SERVER_PORT3 1513 // send wp
...
void movePlane(double y, double angle)
{
   ...
   // double factor =  0.0003;
    //double factor =  0.00003;//orig speed
            ...
            case 0:
              ...
              // printf("Drone 2 Waiting\n");
               ...
               // printf("Got packet\n");
                ...
                //printf("wp[4]: %f\nwp[7]: %f\n",wp_info[4],wp_info[5]);
                if ((wp_info[0] == 3.000) && (wp_info[3] != curr_wp) &&(wp_info[x_i+1] != 0.0) && (wp_info[x_i+2] != 0.0)){
                printf("Got new waypoint 3! (%f,%f)\n",wp_info[x_i+1],wp_info[x_i+2]);
                get_wp = 1;
              ...
              //  int result2 = (int)(wp_info);
               ...
               //waypoint_active = 0;
                ...
                //printf("Moving Block: %f\n",moving_block);
               
               if (wp_info[0] == 0.000){
               drone_min_x = wp_info[1]+19.9352;
               drone_min_y = wp_info[2]+19.9403;
               drone_max_x = wp_info[5]+19.9352;
               drone_max_y = wp_info[6]+19.9403;
               start_x = drone_min_x + 19.9352;
               start_y = drone_min_y + 19.9403;
               //coords[0] = start_x + 19.9352;
               //coords[2] = start_y + + 19.9403;
...
if (((coords[2] + 19.9403) > drone_max_y + 19.9403) || (angle_check > 50 )){
rot[3] = rot[3] + (pi/4);
angle_check = 0;
}

if (((coords[0] + 19.9352) < drone_min_x + 19.9352) || (angle_check > 50 )){
rot[3] = rot[3] + (pi/4);
}

if ((coords[2] + 19.9403) < drone_min_y + 19.9403){
rot[3] = rot[3] + (pi/4);
}
...
// printf("Current Rot: %f\n",rot[3]);
 ...
 // printf("Forward Motion\n");
  ...
  //temp_wp[i++] = 1.3;
  float bound_z = 0.0;
  if (((x_coord != bound_z) && (y_coord != bound_z))){
...
// socket //
  ...
  //pthread_create(&vid,NULL,video,"Data stuff.. \n");
    ...
    drone_node = wb_supervisor_node_get_from_def("Drone4");
