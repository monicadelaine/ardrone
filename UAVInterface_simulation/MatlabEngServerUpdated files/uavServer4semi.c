...
int matCoords[4], interfaceCoords[4];
...
Engine *ep;
mxArray *cpresult = NULL, *c1result = NULL, *c2result = NULL, *c3result = NULL, *c4result = NULL, *dresult = NULL;
double alulFlag[0], start[0], cell_pos[0], cell1[0], cell2[0], cell3[0], cell4[0], mdata[0];
mxArray *m_A, *m_S, *m_CP, *m_C1, *m_C2, *m_C3, *m_C4, *m_D;
char *cpresult2, *dresult2;
double *cresult;
float matData[400][3];
int startEng = 0;
int x1, y11, x2, y2, x3, y3, x4, y4;
int wp_x1[50], wp_y1[50], wp_x2[50], wp_y2[50], wp_x3[50], wp_y3[50], wp_x4[50], wp_y4[50];
...
void toInterface(int x, int y) {
	interfaceCoords[0] = (int)( (x + X_OFFSET) * IPAD_UNIT_X );
	interfaceCoords[1] = (int)( (y + Y_OFFSET) * IPAD_UNIT_Y );

	//printf("toInterface=> x: %i y: %i, intX: %i, intY: %i\n",x,y,interfaceCoords[0],interfaceCoords[1]);
}
			...
			case 0:
				...
				if(n_wp<0) {
					...
					if (gotCoords > 0){ //wp data
						...
						//printf("r: %f,%f ", ix, iy); fflush(stdout);
					} else{ //init coord
					...
					if (pID2 == 1){
						...
						if (ALG_FLAG == 3 && dflag == 1) {
							WP_FLAG1 = 1; 
							wp_x1[wpCnt1] = int_p1x;
							wp_y1[wpCnt1] = int_p1y;
							printf("manual wp1=> x1: %i, y1: %i \n",wp_x1[wpCnt1],wp_y1[wpCnt1]);
							wpCnt1++;		
							//rArray[0] = ix, rArray[1] = iy;	
							//printf("r: %f, %f \n",rArray[0],rArray[1]);
						}
						dflag = 1;
					} else if (pID2 == 2) {
						...
						if (ALG_FLAG == 3) {
							WP_FLAG2 = 1; 
							wp_x2[wpCnt2] = int_p1x;
							wp_y2[wpCnt2] = int_p1y;
							printf("manual wp2=> x2: %i, y2: %i \n",wp_x2[wpCnt2],wp_y2[wpCnt2]);
							wpCnt2++;
							//rArray[2] = ix, rArray[3] = iy;
						}
					} else if (pID2 == 3) {
						...
						if (ALG_FLAG == 3) {
							WP_FLAG3 = 1; 
							wp_x3[wpCnt3] = int_p1x;
							wp_y3[wpCnt3] = int_p1y;
							printf("manual wp2=> x3: %i, y3: %i \n",wp_x3[wpCnt3],wp_y3[wpCnt3]);
							wpCnt3++;
							//rArray[4] = clone_int_p1x, rArray[5] = clone_int_p1x;
						}
					} else if (pID2 == 4) {
						...
						if (ALG_FLAG == 3) {
							WP_FLAG4 = 1; 
							wp_x4[wpCnt4] = int_p1x;
							wp_y4[wpCnt4] = int_p1y;
							printf("manual wp4=> x4: %i, y4: %i \n",wp_x4[wpCnt4],wp_y4[wpCnt4]);
							wpCnt4++;
							//rArray[6] = clone_int_p1x, rArray[7] = clone_int_p1x;
						}
					}
				}
				break;
	...
	while (1) {	
		...
		if (ALG_FLAG != 1) {
			...
			while (1) {
				...
				//float int_r1x, int_r1y, int_r2x, int_r2y, int_r3x, int_r3y, int_r4x, int_r4y, int_wp_id;
