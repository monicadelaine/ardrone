...
int IPAD_FLAG = 1; //1=manual, 2=auto, 3=semiauto
	...
	while(1){
		...
		while (recData < 1)    {
			...
			case 0:
				...
				tposArrary[8] = posArrary4[0];
                                tposArrary[9] = posArrary4[1];
			...
			case 0:
				...
				if(n_wp<0) {
					...
					if (gotCoords < 1){ //init coord
						...
						if (WP_FLAG4 == 1) {
							lastx4 = int_p1x, lasty4=int_p1y;
							printf("wp4=> lastx4: %i, lasty4: %i ",lastx4,lasty4);
						}
					}
	...
	while (1) {
		...
		if (IPAD_FLAG != 1) {
			...
			while (1) {
				...
				printf("p1: %f %f, p2: %f %f, p3: %f %f, p4: %f %f \n",int_p1x, int_p1y, int_p2x, int_p2y, int_p3x, int_p3y, int_p4x, int_p4y);
...
void *posThread4() {
	...
	while(1){
		...
		while (recData < 1) {
			...
			case 0:
				cliLen2 = sizeof(cliAddr2);
				n2 = recvfrom(sd2, posArrary4, sizeof(posArrary4), flags2,(struct sockaddr *) &cliAddr2, &cliLen2);
				x4 = posArrary4[0], y4 = posArrary4[1];
				//printf("p4: %d %d ",x4,y4);
