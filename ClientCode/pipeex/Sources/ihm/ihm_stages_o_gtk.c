...
int32_t copy_rowstide = 0;
uint8_t* copy_data = NULL;
GtkImage *image_copy = NULL;
gchar* buf;
gsize buf_size;
GError* error = NULL;
int set;
...
output_gtk_stage_open( vp_stages_gtk_config_t *cfg, vp_api_io_data_t *in, vp_api_io_data_t *out)
{
    ...
    data_pack1 = vp_os_malloc(57600 * sizeof(uint8_t));
    data_pack2 = vp_os_malloc(57600 * sizeof(uint8_t));
    data_pack3 = vp_os_malloc(57600 * sizeof(uint8_t));
    data_pack4 = vp_os_malloc(57600 * sizeof(uint8_t));
    data_pack5 = vp_os_malloc(57600 * sizeof(uint8_t));
    ship = vp_os_malloc(230400 * sizeof(uint8_t));
    ...
    /*
     while(1)
     {
      if (c > 230400)
      {
        ...
        if ((pixbuf_data[c] == pixbuf_data[c-1] == pixbuf_data[c-2] == pixbuf_data[c-3] == pixbuf_data[c-4] == pixbuf_data[c-5] == pixbuf_data[c-6] == pixbuf_data[c-7] == pixbuf_data[c-8] == pixbuf_data[c-9] == pixbuf_data[c-10] == pixbuf_data[c-11] == pixbuf_data[c-12] == pixbuf_data[c-13] == pixbuf_data[c-14] == pixbuf_data[c-15] == pixbuf_data[c-16] == pixbuf_data[c-17] == pixbuf_data[c-18] == pixbuf_data[c-19] == pixbuf_data[c-20]  ) && (pixbuf_data[c] == 0) && (stop != 1))
        {
            stop = 1;
            num = c;
            printf("buf[%d]: %d, index start = %d\n",c, pixbuf_data[c],num);
...
*/
	  ...
	  //// split package
	//  vp_os_memset(data_pack1,0x0,57600);
    //  vp_os_memset(data_pack2,0x0,57600);
    //  vp_os_memset(data_pack3,0x0,57600);
    //  vp_os_memset(data_pack4,0x0,57600);

   // memcpy(data_pack1, pixbuf_data, 57600 * sizeof(uint8_t));
    /*
    memcpy(data_pack2, pixbuf_data + 57600, 57600 * sizeof(uint8_t));
    memcpy(data_pack3, pixbuf_data + 115200, 57600 * sizeof(uint8_t));
    memcpy(data_pack4, pixbuf_data + 172800, 57600 * sizeof(uint8_t));
    memcpy(data_pack5, pixbuf_data + 230400, 57600 * sizeof(uint8_t));
    */
   // memcpy(ship, pixbuf_data, 230400 * sizeof(uint8_t));
	...
	/*
	  for (i; i < 57600; i++)
	  {
        data_pack1[i] = (uint8_t)pixbuf_data[i];
	  ...
	  for(i; i<115200;i++)
	  {
	      ...
	      data_pack2[count] = (uint8_t)pixbuf_data[i];
	      count++;
	  }
	  ...
	  for (i; i<172800; i++)
	  {
	   ...
	   data_pack3[count] = (uint8_t)pixbuf_data[i];
        count++;
	  }
    ...
    for (i; i<230400; i++)
    {
            data_pack4[count] = (uint8_t)pixbuf_data[i];
           // printf("%d[%d] = %d[%d]\n", data_pack4[count],count, pixbuf_data[i],i);
            count++;
    ...
    for (i; i<288000; i++)
    {
            data_pack5[count] = (uint8_t)pixbuf_data[i];
           // printf("%d[%d] = %d[%d]\n", data_pack4[count],count, pixbuf_data[i],i);
            count++;
           ...
           PRINT("data[8]: %d\n",test[8]);
             PRINT("data[9]: %d\n",test[9]);
               PRINT("data[10]: %d\n",test[10]);
                 PRINT("data[11]: %d\n",test[11]);
*/
      //copy_width = pixbuf_width;
     // copy_height = pixbuf_height;
     // copy_rowstide = pixbuf_rowstride;
    //  copy_data = (uint8_t*)in->buffers[0];


   // int size1 = sizeof(pixbuf_data);

    // memcpy(copy_data, pixbuf_data,sizeof(pixbuf_data));
  ...
  if(pixbuf!=NULL)
  {
     ...
     if (!gdk_pixbuf_save_to_buffer (pixbuf, &buffer, &count, "jpeg",
        ...
        memcpy(data_pack1, buffer, 57600 * sizeof(uint8_t));
        set = 1;


             // gdk_pixbuf_save_to_buffer(pixbuf, &buf, &buf_size, "jpeg", &error, "quality", "100", NULL);
              //buf[0] = (gchar)buf_size;
...
//       point.y += ACQ_HEIGHT / 2;
      ...
      if( point.x >= QVGA_WIDTH || point.x < 0 || point.y >= QVGA_HEIGHT || point.y < 0 )
      {
...
DEFINE_THREAD_ROUTINE(ihm_stages_vision, data)
{
  ...
  jdc.width				= QVGA_WIDTH;
  jdc.height			= QVGA_HEIGHT;
...
*/
DEFINE_THREAD_ROUTINE(stages_vision, data)
{
  C_RESULT res;
