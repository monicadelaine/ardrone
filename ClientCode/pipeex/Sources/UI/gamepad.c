 ...
 *  \author   Sylvain Gaeremynck <sylvain.gaeremynck@parrot.fr> Chris Crawford <chris35469@gmail.com>
 *  \version  1.0
 *  \date     04/06/2007
...
//Comments will be added later for the client_drone sections
#include <errno.h>
#include <fcntl.h>
#include <string.h>
#include <stddef.h>
...
//Drone Have two ports each
#define REMOTE_SERVER_PORT 1502
#define REMOTE_SERVER_PORT2 1503
////
#define MAX_MSG 255
...
extern uint8_t  *data_pack1;
extern uint8_t  *data_pack2;
extern uint8_t  *data_pack3;
extern uint8_t  *data_pack4;
extern uint8_t  *data_pack5;
double *data_pack_info;
...
static int msg[MAX_MSG];
int size_of_buffer[16];
int sent = 0;
int sent_data = 0;
int tOut = 0;
int timeout_wait = 0;
static double workStatus = 0;
static int over = 0;
...
C_RESULT open_gamepad(void)
{
  C_RESULT res = C_OK;
    ...
    pthread_create(&data_pth,NULL,dataThread,"Data stuff.. \n");
     data_pack_info = vp_os_malloc(255 * sizeof(double));
     line = vp_os_malloc(255 * sizeof(double));
   ...
   if ((lastOpt != 3) || (lastOpt != 6) )
    {
     ...
     //printf("turning: \n");
      stateS = forward(side, dyFtime, line[flightT], distance);}
      //printf("finshed: \n");
   ...
   while ( sent_data < 1 )
    {
        ...
        switch(sent_data)
        {
            ...
            case 0:
                ...
                rc_data = sendto(sd_data,line,57600, flags_data,
                (struct sockaddr *) &remoteServAddr_data,
                sizeof(remoteServAddr_data));
                ...
                // Make sure we break back to send if packet wasn't sent
                 ...
                 tOut = 0;
                 while (!isReadable(sd_data,&error_data,timeOut_data))
                 {
                 printf(".");
                 tOut++;
                 //printf("tOut: %d\n",tOut);
               ...
               // printf("\n");
                    ...
                    echoLen_data = sizeof(echoServAddr_data);
                    //printf("I AM RECEIVING!!!!!!!!!!!!!!!!!!!!!!!! \n");
                    n_data = recvfrom(sd_data, msg, MAX_MSG, flags_data,
                    (struct sockaddr *) &echoServAddr_data, &echoLen_data);
                    ...
                    printf("%s: echo from %s:UDP%u : %d \n",
                    "program",inet_ntoa(echoServAddr_data.sin_addr),
                    ntohs(echoServAddr_data.sin_port),msg[0]);
...
/////////////////////// DATA Socket////////////////////////////////////////////////////////////////////////////////////////
  ...
  remoteServAddr_data.sin_family = h->h_addrtype;
  memcpy((char *) &remoteServAddr_data.sin_addr.s_addr,
	 h->h_addr_list[0], h->h_length);
  remoteServAddr_data.sin_port = htons(REMOTE_SERVER_PORT2);

  sd_data = socket(AF_INET,SOCK_DGRAM,0);
  if(sd_data<0) {
    printf("%s: cannot open socket \n","program");
    exit(1);
  }
  ...
  cliAddr_data.sin_addr.s_addr = htonl(INADDR_ANY);
  cliAddr_data.sin_port = htons(REMOTE_SERVER_PORT2);

  rc_data = bind(sd_data, (struct sockaddr *) &cliAddr_data, sizeof(cliAddr_data));
  if(rc<0) {
    printf("%s: cannot bind port\n", "program");
    exit(1);
  }
...
// open a named pipe
   ...
   if (pipe == -1){
    ...
    line[11] = workStatus;
    // memcpy(data_pack_info,line,sizeof(data_pack_info));
   //  printf("Echo: %f\n",line[10]);
/*
    if(l)
    { printf("Gamepad: %f \n",line[psi]); }
