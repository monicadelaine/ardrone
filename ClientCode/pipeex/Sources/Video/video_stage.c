...
static GtkWidget *ImageWin, *VideoStream_VBox=NULL;
...
C_RESULT output_gtk_stage_open( void *cfg, vp_api_io_data_t *in, vp_api_io_data_t *out)
{
   ...
   gtk_container_set_border_width(GTK_CONTAINER(ImageWin), 200);
   VideoStream_VBox = gtk_vbox_new(FALSE, 0);
    ...
    if( image == NULL && pixbuf!=NULL)
		  {
				image  = (GtkImage*) gtk_image_new_from_pixbuf( pixbuf );
						if(GTK_IS_WIDGET(ImageWin))
						if (GTK_IS_WIDGET(VideoStream_VBox))
							gtk_container_add( GTK_CONTAINER( VideoStream_VBox ), (GtkWidget*)image );
		  }
